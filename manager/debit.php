...
<body>
    ...
    <div class="search-bar" id="the-search-bar">
       ...
       <a class="add-button" href="add_payee.php">Add</a>
...
first_name, last_name, account_no FROM customer
                        WHERE customer_id=".$row["benef_customer_id"]." AND (first_name LIKE '%$search%'
      
                  OR last_name LIKE '%$search%' OR CONCAT(first_name, ' ', last_name) LIKE '%$search%')";
                    }
        else 
{    $sql1 = "SELECT customer_id, first_name, last_name, account_no FROM customer
                        WHERE customer_id=".$row["benef_customer_id"]." 
AND account_no LIKE '$search'";
                    }
                }
                else {
        
            $sql1 = "SELECT customer_id, first_name, last_name, account_no
                             FROM customer WHERE customer_id=".$row["benef_customer_id"];
...
<p id="id"><?php echo $i . "."; ?></p>
   ...
   <button onclick="dropdown_func(<?php echo $i ?>)" class="dropbtn"></button>
     ...
     <div id="dropdown<?php echo $i ?>" class="dropdown-content">
                         ...
                         <a href="send_funds.php?customer_id=<?php echo $row1["customer_id"] ?>">Send</a>
  
                              <a href="delete_payee.php?customer_id=<?php echo $row1["customer_id"] ?>"
