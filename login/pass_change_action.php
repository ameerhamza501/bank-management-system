...
include "account_holdersidebar.php";
    ...
    /*  Set appropriate error number if errors are encountered.
        ...
        -1 = conection Error.
         0 = Successful Change.
         1 = Old pasword is incorrect.
   
      2 = paswords do not match.
         3 = Old and new paswords are same. */
    $err_no = -1;
   ...
   if ($type == "pasword") {
        $sql0 = "SELECT * FROM customer WHERE customer_id=".$id." AND pwd='".$old_pwd."'";
        $info_string = "pasword";
   
 }

    if ($type == "pin") {
        $sql0 = "SELECT * FROM customer WHERE customer_id=".$id." AND pin='".$old_pwd."'";
        $info_string = "PIN";
    }
  ...
  $result0 = $conn->query($sql0);
    if (($result0->num_rows) > 0) {
      ...
      if ($type == "pasword") {
                    $sql1 = "UPDATE customer SET pwd = '$new_pwd' WHERE customer_id=".$id;
                }
                
if ($type == "pin") {
                    $sql1 = "UPDATE customer SET pin = '$new_pwd' WHERE customer_id=".$id;
                }
...
<body>
    <div class="flex-container">
       ...
       <?php
            if ($err_no == -1) { ?>
                <p id="info"><?php echo "conection Error ! Please try again later.\n"; ?></p>
